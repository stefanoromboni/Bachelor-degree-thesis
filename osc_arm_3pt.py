...
'''
Risolviamo il problema dell'oscillatore armonico, definito teoricamente su tutto R.
...
'''
Andiamo a definire una funzione chiamata "osc_arm3", che prendera come parametri iniziali
...
plt.plot(xx,ff(xx,*pars), color='red', label='best-fit')
...
for i in range(0, len(n)):
    w, v, x = osc_arm3(-10, 10, n[i])
    M[i]=np.sort(w)[:9]

M = np.transpose(M)


# file=open('/Users/stefanoromboni/Desktop/Università/Tesi/Codici python/oscillatore armonico/tab_eigval.txt', 'w')
# for i in range (0, len(n)):
#     file.write('$%.5f$ \t&\t $%.5f$ \t&\t $%.5f$ \t&\t $%.5f$ \t&\t $%.5f$ \t&\t $%.5f$ \t&\t $%.5f$ \t \\\\ \n' %((2*i+1),M[i,0], M[i,1], M[i,2], M[i,3], M[i,4], M[i,5]) )
# file.close()
...
plt.plot(n[:5], np.abs(1-fond), color='limegreen', linestyle='--', marker='o', label='3pt+RE')
